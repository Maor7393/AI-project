...
def run_dijkstra(graph, current_vertex):
	return {},{}
...
class Greedy(agent.Agent):
	...
	def act(self, graph):
		...
		if len(self.sequence) == 0:
			distances, prevs = run_dijkstra(graph, self.current_vertex)
			...
			if len(self.sequence) is 0:
				...
				move_description = move_description + "SAVED " + str(amount_of_people)
			return move_description
