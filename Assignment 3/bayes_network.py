...
def enumeration_all(variables, evidence, bayes_network):
	...
	if has_val:
		...
		prob_Y_false = Y_node.table.get_probability_given_parents(Y,False, Y_parents) * enumeration_all(variables[1:], evidence.union({(Y, False)}), bayes_network)
		prob_Y_true = Y_node.table.get_probability_given_parents(Y,True, Y_parents) * enumeration_all(variables[1:], evidence.union({(Y, True)}), bayes_network)
...
def enumeration_ask(x_query: list, evidence: set, bayes_network: BayesNetwork):
	...
	for assignment in x_query_possible_values:
		...
		distribution_x[assignment] = round(prob,2)
