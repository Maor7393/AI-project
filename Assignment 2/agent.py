...
class Agent:
    ...
    def __init__(self, max_starting_vertex, min_starting_vertex, vertices_status, act_sequence, other_agent,
                 comparator):
        self.state = s.State(s.Location(max_starting_vertex, 0, max_starting_vertex),
                             s.Location(min_starting_vertex, 0, min_starting_vertex), vertices_status, 0, 0, 0)
    ...
    # (maxAgentScore, minAgentScore)

    def adversarial_comparator(self, val1, val2):
        pass

    def semi_cooperative_comparator(self, val1, val2):
        pass

    def fully_cooperative_comparator(self, val1, val2):
        pass
...
class MaxAgent(Agent):
    ...
    def minimax(self, state: s.State, WORLD: Graph):
        ...
        for next_state in state.successor("MAX", WORLD):
            ...
            if best_value is None:
                ...
                best_value = self.comparator(best_value, value_of_new_state)
        return [best_edge[0], best_edge[1], best_edge[2]]

    # (maxAgentScore, minAgentScore)

    def adversarial_comparator(self, best_value_tup, new_state_tup):
    ...
    def semi_cooperative_comparator(self, best_value_tup, new_state_tup):
...
class MinAgent(Agent):
    ...
    def set_new_location(self, location_params):
        self.state.min_agent_current_location.set_location_parameters(location_params[0], location_params[1],
                                                                      location_params[2])
    ...
    def minimax(self, state: s.State, WORLD: Graph):
        ...
        for next_state in state.successor("MIN", WORLD):
            ...
            current_edge = WORLD.get_edge(next_state.min_agent_current_location.prev,
                                          next_state.min_agent_current_location.successor)
            if best_value is None:
                best_value = value_of_new_state
                best_edge = current_edge
    ...
    def fully_cooperative_comparator(self, best_value_tup, new_state_tup):
        best_value = best_value_tup[0] + best_value_tup[1]
        new_value = new_state_tup[0] + new_state_tup[1]
